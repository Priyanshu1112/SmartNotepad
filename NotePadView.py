...
class Notepad:
    def __init__(self):
        ...
        #self.new_file = None
        self.root = tk.Tk()
        self.root.geometry('1200x800+100+5')
        ...
        self.use_time_obj = use_time(self.status_bar)
        self.use_time_obj.start()
    ...
    def set_style(self):
        self.selected_style = ttk.Style()
        self.selected_style.configure('s.TButton', background='#000000', relief='sunken')
        self.deselected_style = ttk.Style()
        self.deselected_style.configure('d.TButton', background='SystemButtonFace', relief='raised')
    ...
    def check_db_status(self):
        ...
        if self.db_status:
            ...
            tk.messagebox.showerror("Database Error", "Please Connect with the database")

    def exit_func(self, event=None):
        try:
            self.text_end = self.text_editor.get(1.0, tk.END)
            if self.text_end != '\n':
                if self.text_start != self.text_end:
                    mbox = tk.messagebox.askyesnocancel('Warning', 'Do you want to save the changes?')
                    if mbox is True:
                        content = self.text_editor.get(1.0, tk.END)
                        self.file_controller.save_changes(content, self.path)
            self.use_time_obj.stop()
            self.root.destroy()
        except:
    ...
    def save_file(self, event=None):
        try:
            ...
            self.text_start = content
            #print(content, len(content))
        except:
            tk.messagebox.showerror('Error', 'Cannot save the file')
            print(traceback.format_exc())
    ...
    def open_file(self, event=None):
       try:
           url=tk.filedialog.askopenfilename()
           self.path = url
           self.msg, self.base = self.file_controller.read_file(url)
           self.file_name = self.base
           #print(self.base)
           #print(self.is_secure_file_path(self.base, url))
           if self.is_secure_file_path(self.base, url):
               if self.get_file_pwd() == self.db_controller.get_file_pwd(self.base):
                   self.text_editor.delete(1.0, tk.END)
                   self.text_editor.insert(1.0, self.msg)
                   self.text_start = self.text_editor.get(1.0, tk.END)
                   self.root.title(self.base)
               else:
                   tk.messagebox.showerror('Wrong Password', 'Wrong Password!, Please Try Again!!')
           else:
               self.text_editor.delete(1.0, tk.END)
               self.text_editor.insert(1.0, self.msg)
               self.text_start = self.text_editor.get(1.0, tk.END)
    ...
    def changed(self, event=None):
        if self.text_editor.edit_modified():
            ...
            use_time = text[text.find('Use'):]
            if characters != 0:
                self.status_bar.config(text=f'Characters:{characters}     Words:{words}       {use_time}')
                self.text_editor.edit_modified(False)
            else:
                self.status_bar.config(text=use_time)
                self.text_editor.edit_modified(False)
    ...
    def say_something(self):
        try:
            ...
            print(self.take_audio.lower())
            if self.take_audio == '':
                tk.messagebox.showinfo('Say Again', 'Speech not recognized')
            elif self.take_audio.lower() in 'hey mojo open file':
                self.open_file()
            elif self.take_audio.lower() in 'hey mojo save file':
                self.save_file()
            elif self.take_audio.lower() in 'hey mojo new file':
                self.new_file()
            elif self.take_audio.lower() in 'hey mojo file save':
                self.save_as()
            elif self.take_audio.lower() in 'hey mojo close app':
                self.exit_func()
            elif self.take_audio.lower() in 'hey mojo text-bold':
                self.change_bold()
            elif self.take_audio.lower() in 'hey mojo text-italic':
                self.change_italic()
            elif self.take_audio.lower() in 'hey mojo search':
                self.find_func()
            elif self.take_audio.lower() in 'hey mojo hide statusbar':
                self.hide_statusbar()
            elif self.take_audio.lower() in 'hey mojo hide toolbar':
                self.hide_toolbar()
            else:
    ...
    def align_left(self):
        self.align_left_btn['style'] = 's.TButton'
        self.align_right_btn['style'] = self.align_center_btn['style'] = 'd.TButton'
    ...
    def align_center(self):
        self.align_center_btn['style'] = 's.TButton'
        self.align_left_btn['style'] = self.align_right_btn['style'] = 'd.TButton'
    ...
    def align_right(self):
        self.align_right_btn['style'] = 's.TButton'
        self.align_left_btn['style'] = self.align_center_btn['style'] = 'd.TButton'
        text_content = self.text_editor.get(1.0, 'end')
    ...
    def change_font(self, event=None):
        ...
        my_font = tkFont.Font(self.text_editor, self.text_editor.cget('font'))
        my_font.configure(family=self.current_font_family)
        self.text_editor.configure(font=my_font)


    def change_fontsize(self, event=None):
        self.current_font_size = self.font_size.get()
        my_font = tkFont.Font(self.text_editor, self.text_editor.cget('font'))
        my_font.configure(size=self.current_font_size)
        self.text_editor.configure(font=my_font)

    def change_bold(self):
        my_font = tkFont.Font(self.text_editor, self.text_editor.cget('font'))
        if self.bold_val == 'normal':
            self.bold_val = 'bold'
            self.bold_btn['style'] = 's.TButton'
        elif self.bold_val == 'bold':
            self.bold_val = 'normal'
            self.bold_btn['style'] = 'd.TButton'
        my_font.configure(weight=self.bold_val)
        my_font.configure(underline=self.underline_val)
        my_font.configure(slant=self.italic_val)
        self.text_editor.configure(font=my_font)

    def change_italic(self):
        my_font = tkFont.Font(self.text_editor, self.text_editor.cget('font'))
        if self.italic_val == 'roman':
            self.italic_val = 'italic'
            self.italic_btn['style'] = 's.TButton'
        elif self.italic_val == 'italic':
            self.italic_val = 'roman'
            self.italic_btn['style'] = 'd.TButton'
        my_font.configure(weight=self.bold_val)
        my_font.configure(underline=self.underline_val)
        my_font.configure(slant=self.italic_val)
        self.text_editor.configure(font=my_font)
    ...
    def change_underline(self):
        my_font = tkFont.Font(self.text_editor, self.text_editor.cget('font'))
        if self.underline_val:
            self.underline_val = False
            self.underline_btn['style'] = 'd.TButton'
        elif not self.underline_val:
            self.underline_val = True
            self.underline_btn['style'] = 's.TButton'
        my_font.configure(weight=self.bold_val)
        my_font.configure(underline=self.underline_val)
        my_font.configure(slant=self.italic_val)
        self.text_editor.configure(font=my_font)
    ...
    def secure_file(self, event=None):
        if self.db_status:
            ...
            self.file_list = tk.Listbox(self.file_dialogue)
            #self.fileList.place(relx=0.0, rely=0.30, relheight=1, relwidth=0.972)
            self.file_list.pack(fill='both', expand=True)
            self.file_list.configure(background='white')
            self.file_list.configure(disabledforeground='#a3a3a3')
            self.file_list.configure(font='Lato')
            self.file_list.configure(foreground='black')
            self.file_list.configure(highlightbackground='#d9d9d9')
            self.file_list.configure(highlightcolor="#d9d9d9")
            self.file_list.configure(selectbackground="#c4c4c4")
            self.file_list.configure(selectforeground="black")
            self.file_list.configure(width=10)
            self.file_list.configure(yscrollcommand=scrollbar.set)
            scrollbar.configure(command=self.file_list.yview)
            self.file_list.bind("<Double-1>", self.list_double_click)
            ...
            for values in self.files.keys():
                ...
                self.file_list.insert('end', full_file)

            self.file_dialogue.mainloop()

        else:
            tk.messagebox.showerror('Databse Error', 'Please connect with the database')

    def list_double_click(self,e):
        self.open_secure_file()

    def open_secure_file(self):
       self.sel_file_index_tuple = self.file_list.curselection()
       if len(self.sel_file_index_tuple)!=0:
            self.full_file_name = self.file_list.get(self.sel_file_index_tuple[0])
    ...
    def remove_secure_file(self):
        ...
        if len(self.sel_file_index_tuple)!=0:
            ...
            self.full_file_name = self.file_list.delete(self.sel_file_index_tuple[0])
            messagebox.showinfo('Success!', 'File removed from secure files')
        else:
            messagebox.showerror('Error!', 'Please select a file')
    ...
    def is_secure_file_path(self, file_name, file_path):
        return self.db_controller.is_secure_file_path(file_name, file_path)

    def get_file_pwd(self):
        access_pwd = simpledialog.askstring('Password', 'Please Enter Access Password')
        return access_pwd
...
class use_time(threading.Thread):
    ...
    def run(self):
        while self.run_thread:
            ...
            self.current_time = datetime.datetime.strptime(self.current_time, '%H:%M:%S').time()
            #print('Current time',self.current_time)
            time_used = datetime.datetime.combine(datetime.date.today(),self.current_time) - datetime.datetime.combine(datetime.date.today(), self.start_time)
            text = self.status_bar['text']
            #print(text)
            if text == 'Status Bar':
                text = ''
            pos = text.find('Use')
            use_time = 'Use time :' + str(time_used)+'      Date :'+str(self.current_date)
            if pos == -1:
                text+=' '+use_time
            else:
                text = text[:pos]
